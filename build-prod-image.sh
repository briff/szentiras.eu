...
FILE_PATH=$DIR/szeu-app-prod-$GIT_SHORT_HASH.tar.gz
...
if [ -f "$FILE_PATH" ]; then
    ...
    APP_DOMAIN=szentiras.eu GIT_COMMIT_HASH=$GIT_FULL_HASH docker compose -f docker-compose.prod.yml build app
...
ls -t dist/szeu-app-prod-*.tar.gz | tail -n +6 | xargs rm -f 2>/dev/null || true
